...
class NotesViewModel @Inject constructor(
    ...
    fun onEvent(event:NotesEvent){
        when(event){
            is NotesEvent.Order -> {
                if (state.value.noteOrder::class == event.noteOrder::class && state.value.noteOrder== event.noteOrder.orderType){
                    return
                }
            ...
            is NotesEvent.RestoreNote -> {
                viewModelScope.launch {
                    noteUseCases.insertNoteUseCase(lastDeletedNote ?: return@launch)
